...
type NotifStatus = 'sent' | 'failed' | 'no_email';
...
async function getPatientEmail(patientId: string, patientCedula: string): Promise<string> {
    if (patientId) {
        try {
            const snap = await db.collection('patients').doc(patientId).get();
            if (snap.exists) return String(snap.data()?.email || '').trim();
        } catch (err) {
    ...
    if (patientCedula) {
        try {
            const q = await db.collection('patients')
                ...
                .limit(1)
                .get();
            if (!q.empty) return String(q.docs[0].data().email || '').trim();
        } catch (err) {
...
async function getDoctorName(doctorUid: string): Promise<string> {
    if (!doctorUid) return 'su médico';
    try {
        const snap = await db.collection('users').doc(doctorUid).get();
        if (snap.exists) return String(snap.data()?.nombre || 'su médico').trim();
    } catch { /* best effort */ }
    return 'su médico';
}
...
export const onAppointmentCreated = onDocumentCreated(
    ...
    async (event) => {
        const appt = event.data?.data() as AppointmentData | undefined;
        if (!appt) return;
